...
class KennyLoggins:
    ...
    def get_logger(self, app_name=None, file_name="kenny_loggins", log_level=logging.INFO):
        ...
        formatter = logging.Formatter(
            '%(asctime)s log_level=%(levelname)s pid=%(process)d tid=%(threadName)s file="%(filename)s" function="%(funcName)s" line_number="%(lineno)d" version="{}" %(message)s'.format(gmail_version.__version__))
...
class Utilities:
    ...
    def get_credential(self, realm, cuser):
        ...
        """
        try:
            self._log.info("realm={} cuser={} app={} session_key={}".format(realm, cuser, self._app_name, self._session_key))
